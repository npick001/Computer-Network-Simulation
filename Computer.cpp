...
void Computer::GenerateMessageEM()
{
    ...
    int finalDestination = rand() % _computerNetwork->GetNetworkSize();

    while (finalDestination == _id) {
        finalDestination = rand() % _computerNetwork->GetNetworkSize();
    }
...
void Computer::ProcessMessage(Message *message) {
    int finalDestination = message->getDestination()->getId();
            ...
            if (path.size() >= 1) {
                ...
                SimulationExecutive::ScheduleEventIn(0.0, new SendMessageEA(this, message));
...
{
    SimulationExecutive::ScheduleEventIn(0.0, new ArriveEA(_nextComputer, message));
}
